...
class Pendidikan extends BaseController
{
	public function index()
	{
		...
		if($session->has('username')){
			...
			return view("daftar_pendidikan",$data);
		}else{
			return view('login');
	...
	public function edit(){
		...
		if($mode=='edit'){
		$builder->where('id_pendidikan',$post['id']);
		unset($post['id']);
	...
	public function json_pendidikan(){
		...
		$sidx = $_POST['sidx']; // get index row - i.e. pendidikan click to sort
		...
		$builder = $db->table('tbl_pendidikan a');
		...
		foreach($get as $row){
			...
			$responce->rows[$i]['id'] = $row['id_pendidikan'];
			
			$responce->rows[$i]['cell'] = array($row['id_pendidikan'], $row['nama_pendidikan']);
			$i++;
		}
